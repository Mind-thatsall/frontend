				...
				<div className="flex flex-col justify-end w-full h-full overflow-hidden shadow-xl md:w-2/3 md:h-2/3 bg-zinc-700/20 rounded-xl ring-1 ring-inset ring-white/10">
							...
							? chat.map((msg, msgIndex) => (
									<div
										...
										<span
											...
											style={{ backgroundColor: msg.colorAvatar }}></span>
										<div>
											<h2 className="mb-2 font-bold text-white">{msg.username}</h2>
					...
					<form action="" className="m-4" onSubmit={addMsg}>
						<div className="flex gap-2">
							<input
								type="text"
								className="w-4/5 p-2 text-white rounded-md lg:w-11/12 bg-zinc-600 highlight-white"
								onChange={(e) => setMsg(e.target.value)}
								id="InputChat"
								placeholder="Message..."
							/>
							<button
								className="w-1/6 p-2 text-white rounded-md lg:w-1/12 bg-zinc-600 highlight-white"
								type="submit">
								Send
							</button>
						</div>
